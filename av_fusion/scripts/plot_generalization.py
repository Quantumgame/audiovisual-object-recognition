...
from single_modality_generalization_roc import *
from meta_generalization_roc import *
...
if __name__ == "__main__":
	...
	roc_auc = auc(fpr, tpr)	
	plt.plot(fpr, tpr, label=u'Curva ROC Áudio (área = %.4f)'%roc_auc)

	fpr, tpr, thresholds = single_modality_classification('v')#single_modality_classification(10,'v')
	roc_auc = auc(fpr, tpr)
	plt.plot(fpr, tpr, label=u'Curva ROC Vídeo (área = %.4f)'%roc_auc)

	fpr, tpr, thresholds = single_modality_classification('av')
	roc_auc = auc(fpr, tpr)
	plt.plot(fpr, tpr, label=u'Curva ROC FAV (área = %.4f)'%roc_auc)

	fpr, tpr, thresholds = meta_classification(a_k, v_k)
	roc_auc = auc(fpr, tpr)
	plt.plot(fpr, tpr, label=u'Curva ROC MAV (área = %.4f)'%roc_auc)
